...
--
-- Host: 127.0.0.1
-- Tempo de geração: 03-Dez-2025 às 17:55
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `P1` (IN `p_data_inicio` DATE, IN `p_data_fim` DATE, IN `p_aluno_id` INT, IN `p_empresa_id` INT, IN `p_estabelecimento_id` INT, IN `p_formador_id` INT)   BEGIN
    ...
    ELSE
        ...
        INSERT INTO estagio (
            ...
            estabelecimento_empresa_id, estabelecimento_id, formador_id
        )
        VALUES (
            p_data_inicio, p_data_fim, p_aluno_id, 
            p_empresa_id, p_estabelecimento_id, p_formador_id
        );
    END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` FUNCTION `F2` (`p_aluno_id` INT, `p_data_inicio` DATE, `p_peso_empresa` DECIMAL(5,2), `p_peso_escola` DECIMAL(5,2), `p_peso_relatorio` DECIMAL(5,2), `p_peso_procura` DECIMAL(5,2)) RETURNS DECIMAL(10,2) DETERMINISTIC READS SQL DATA BEGIN
    ...
    INTO 
        v_n_empresa, v_n_escola, v_n_relatorio, v_n_procura
    FROM estagio
    WHERE 
        aluno_id = p_aluno_id 
...
(1, 1, 1, 4, '2024-02-01', '2024-05-01', 0, 3, 0, 2, 1.25, 4),
(1, 1, 3, 4, '2025-12-03', '2025-12-03', NULL, 0, 0, 0, NULL, NULL),
...
--
-- Estrutura stand-in para vista `q2`
-- (Veja abaixo para a view atual)
--
DROP VIEW IF EXISTS `q2`;
CREATE TABLE `q2` (
`Nome_Empresa` varchar(150)
,`Media_Notas` varchar(417)
);

-- --------------------------------------------------------

--
-- Estrutura stand-in para vista `q3`
-- (Veja abaixo para a view atual)
--
...
--
-- Estrutura stand-in para vista `q4`
-- (Veja abaixo para a view atual)
--
...
--
-- Estrutura stand-in para vista `q5`
-- (Veja abaixo para a view atual)
--
...
--
-- Estrutura stand-in para vista `q6`
-- (Veja abaixo para a view atual)
--
DROP VIEW IF EXISTS `q6`;
CREATE TABLE `q6` (
`Nome_Formador` varchar(150)
,`Media_Formador` varchar(417)
,`Media_Global` varchar(417)
);
...
(0, 'teste.novo', '123', 'Teste novo', 'aluno'),
...
CREATE TABLE `v1` (
`Nome_Formador` varchar(150)
,`Num_Estagios` bigint(21)
,`Media_Formador` varchar(417)
,`Media_Geral` varchar(417)
);
...
DROP VIEW IF EXISTS `q1`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `q1`  AS SELECT `u`.`nome` AS `Nome_Formador`, count(`e`.`formador_id`) AS `Total_Estagios` FROM ((`formador` `f` join `utilizador` `u` on(`f`.`utilizador_id` = `u`.`utilizador_id`)) join `estagio` `e` on(`f`.`utilizador_id` = `e`.`formador_id`)) GROUP BY `f`.`utilizador_id`, `u`.`nome` HAVING count(`e`.`formador_id`) > 1 ;

-- --------------------------------------------------------

--
-- Estrutura para vista `q2`
--
DROP TABLE IF EXISTS `q2`;

DROP VIEW IF EXISTS `q2`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `q2`  AS SELECT `emp`.`firma` AS `Nome_Empresa`, format(avg(`est`.`nota_empresa`),2) AS `Media_Notas` FROM (`empresa` `emp` join `estagio` `est` on(`emp`.`empresa_id` = `est`.`estabelecimento_empresa_id`)) GROUP BY `emp`.`empresa_id`, `emp`.`firma` HAVING avg(`est`.`nota_empresa`) >= 14 ;

-- --------------------------------------------------------

--
-- Estrutura para vista `q3`
--
DROP TABLE IF EXISTS `q3`;

DROP VIEW IF EXISTS `q3`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `q3`  AS SELECT `e`.`firma` AS `Nome_Empresa`, count(`c`.`produto_id`) AS `Total_Produtos` FROM (`empresa` `e` join `comercializa` `c` on(`e`.`empresa_id` = `c`.`estabelecimento_empresa_id`)) GROUP BY `e`.`empresa_id`, `e`.`firma` HAVING count(`c`.`produto_id`) >= 1 ;

-- --------------------------------------------------------

--
-- Estrutura para vista `q4`
--
DROP TABLE IF EXISTS `q4`;

DROP VIEW IF EXISTS `q4`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `q4`  AS SELECT `e`.`firma` AS `Nome_Empresa`, count(`est`.`aluno_id`) AS `Total_Estagios` FROM (`empresa` `e` join `estagio` `est` on(`e`.`empresa_id` = `est`.`estabelecimento_empresa_id`)) GROUP BY `e`.`empresa_id`, `e`.`firma` HAVING count(`est`.`aluno_id`) >= 1 ;

-- --------------------------------------------------------

--
-- Estrutura para vista `q5`
--
DROP TABLE IF EXISTS `q5`;

DROP VIEW IF EXISTS `q5`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `q5`  AS SELECT `c`.`designacao` AS `Nome_Curso`, count(`t`.`turma_id`) AS `Total_Turmas` FROM (`curso` `c` join `turma` `t` on(`c`.`curso_id` = `t`.`curso_id`)) GROUP BY `c`.`curso_id`, `c`.`designacao` HAVING count(`t`.`turma_id`) > (select count(0) from `turma`) / (select count(0) from `curso`) ;

-- --------------------------------------------------------

--
-- Estrutura para vista `q6`
--
DROP TABLE IF EXISTS `q6`;

DROP VIEW IF EXISTS `q6`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `q6`  AS SELECT `u`.`nome` AS `Nome_Formador`, format(avg(`e`.`nota_final`),2) AS `Media_Formador`, format((select avg(`estagio`.`nota_final`) from `estagio`),2) AS `Media_Global` FROM ((`formador` `f` join `utilizador` `u` on(`f`.`utilizador_id` = `u`.`utilizador_id`)) join `estagio` `e` on(`f`.`utilizador_id` = `e`.`formador_id`)) GROUP BY `f`.`utilizador_id`, `u`.`nome` HAVING avg(`e`.`nota_final`) > (select avg(`estagio`.`nota_final`) from `estagio`) ;
...
DROP VIEW IF EXISTS `v1`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v1`  AS SELECT `u`.`nome` AS `Nome_Formador`, count(`e`.`aluno_id`) AS `Num_Estagios`, format(avg(`e`.`nota_final`),2) AS `Media_Formador`, format((select avg(`estagio`.`nota_final`) from `estagio`),2) AS `Media_Geral` FROM ((`formador` `f` join `utilizador` `u` on(`f`.`utilizador_id` = `u`.`utilizador_id`)) join `estagio` `e` on(`f`.`utilizador_id` = `e`.`formador_id`)) GROUP BY `f`.`utilizador_id`, `u`.`nome` ;
...
DROP VIEW IF EXISTS `v2`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v2`  AS SELECT `emp`.`firma` AS `Nome_Empresa`, `c`.`designacao` AS `Nome_Curso`, format(avg(`e`.`nota_final`),2) AS `Media_Notas` FROM ((((`empresa` `emp` join `estagio` `e` on(`emp`.`empresa_id` = `e`.`estabelecimento_empresa_id`)) join `aluno` `a` on(`e`.`aluno_id` = `a`.`utilizador_id`)) join `turma` `t` on(`a`.`turma_id` = `t`.`turma_id`)) join `curso` `c` on(`t`.`curso_id` = `c`.`curso_id`)) GROUP BY `emp`.`empresa_id`, `emp`.`firma`, `c`.`curso_id`, `c`.`designacao` ORDER BY `emp`.`firma` ASC, `c`.`designacao` ASC ;
